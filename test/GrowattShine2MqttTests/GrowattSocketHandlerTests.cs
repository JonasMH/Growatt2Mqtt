...
public class GrowattSocketHandlerTests
{
    ...
    private readonly Mock<IGrowattSocket> _growattSocketMock;

    public GrowattSocketHandlerTests()
    {
        _growattSocketMock = new Mock<IGrowattSocket>();
        var systemClockMock = new Mock<IClock>();

        systemClockMock
            .Setup(x => x.GetCurrentInstant())
            .Returns(Instant.FromUtc(2022, 09, 09, 08, 03, 04));

        var timeZoneProviderMock = new Mock<IDateTimeZoneProvider>();
        timeZoneProviderMock.Setup(x => x.GetSystemDefault())
            .Returns(DateTimeZone.Utc);

        _sut = new GrowattSocketHandler(
            new Mock<ILogger<GrowattSocketHandler>>().Object,
            new Mock<IGrowattToMqttHandler>().Object,
            new GrowattTelegramParser(new Mock<ILogger<GrowattTelegramParser>>().Object, new GrowattTelegramEncrypter()),
            new Mock<IGrowattMetrics>().Object,
            systemClockMock.Object,
            timeZoneProviderMock.Object,
            _growattSocketMock.Object);
    }
    ...
    public async Task HandleMessageAsync_ShouldAckPing()
    {
        ...
        var lastSentBuffer = (ArraySegment<byte>)_growattSocketMock.Invocations.Last().Arguments[0];
    ...
    public async Task HandleMessageAsync_ShouldAckData3()
    {
        ...
        var lastSentBuffer = (ArraySegment<byte>)_growattSocketMock.Invocations.First().Arguments[0];
    ...
    public async Task SendTelegramAsync_GrowattDataloggerQueryTelegram()
    {
        ...
        var result = (ArraySegment<byte>)_growattSocketMock.Invocations.Last().Arguments[0];
